...
struct ThreadData {
slong start, stop, onedivsumstep, id, prec;
}; 
...
void* Ht_Riemann_sum_thread(void *voidData)
{
    ...
    slong onedivsumstep=data->onedivsumstep;
    slong id=data->id;
	...
	//printf("\n inthread id: %ld, start: %ld ,stop: %ld ", id, start, stop);
    for (j = start; j < stop; j++)
    {
		//Positive side of sum 0 .. intlim
		arb_set_d(i, j);
		arb_div_si(i, i, onedivsumstep, prec);
		arb_get_mid_arb(i, i);
		...
		//Negative side of sum -1 .. -intlim (exclude 0)
		if (!(j == 0))
		{
			arb_set_d(i, -j);
			arb_div_si(i, i, onedivsumstep, prec);
...
void
Ht_Riemann_sum_new(acb_struct rs[3], slong sumlim, slong onedivsumstep, slong numthreads, slong prec)
{
    arb_t a, b;
    arb_init(a);
    arb_init(b);
    ...
    threadtasks = (sumlim*onedivsumstep)/numthreads;
		
    for (i = 0; i < numthreads; i++)
    {
    data[i].start= i*threadtasks;
    data[i].stop= (i+1)*threadtasks;
    data[i].onedivsumstep = onedivsumstep;
    data[i].id= i;
    data[i].prec=prec;
	//printf("\n id: %ld, start: %ld ,stop: %ld ", i, data[i].start, data[i].stop);
    }

    data[numthreads-1].stop = sumlim * onedivsumstep + 1;
    ...
    acb_div_si(rs + 0, rs + 0, onedivsumstep, prec);
    ...
    acb_div_si(rs + 1, rs + 1, onedivsumstep, prec);
	...
	acb_neg(rs + 2, rs + 2);
    acb_div_si(rs + 2, rs + 2, onedivsumstep, prec);
...
void
Establish_z_accent_k(acb_t zder, arb_t x, arb_t y, slong sumlim, slong onedivsumstep,slong prec)
{	
	acb_struct s[3];
    acb_init(s);
    acb_init(s + 1);
	...
	Ht_Riemann_sum_new(s, sumlim, onedivsumstep, numthreads, prec);
...
void
Initial_fill_with_zeros_at_t0(slong out, slong kl, slong kh, slong ks, slong sumlim, slong onedivsumstep, slong sw, slong prec)
{    
	slong i, res; res = 0;
	...
	for (i = kl; i < kh + 1; i++)
    {
		...
		Establish_z_accent_k(zder, x, y, sumlim, onedivsumstep, prec);
		acb_get_real(x, zder);
		arb_zero(y);
...
// f(z) = Ht(z) / Ht'(z)
void
f_Ht_frac1(acb_t logder, slong sumlim, slong onedivsumstep, slong prec)
{
    acb_struct s[3];
    acb_init(s);
    acb_init(s + 1);
...
void
Newton_Raphson(acb_t x1, acb_t xx, slong sumlim, slong onedivsumstep, arb_t acc, slong prec)
{
    arb_t a, b;
    arb_init(a);
    arb_init(b);
    ...
    {
        acb_set(z, x0);
        f_Ht_frac1(h, sumlim, onedivsumstep, prec);
...
void
FindRoots(acb_t root, arb_t xs, arb_t ys, slong sumlim, slong onedivsumstep, arb_t acc, slong prec)
{
	...
	Newton_Raphson(root, x0, sumlim, onedivsumstep, acc, prec);
...
slong
Establish_settings_Riemann_sum(slong sumlimtarget, arb_t acc, slong kh, arb_t te, slong onedivsumstep, slong numthreads, slong prec)
{
	slong res; res = 0;
	...
	//establish the maximum x value of the range (Ht has 2 times the x of a zeta zero) 
	fmpz_set_si(n, kh);
	acb_dirichlet_zeta_zero(root, n, prec);
	acb_get_imag(xe, root);
	arb_mul_2exp_si(xe, xe, 1);
	...
	arb_pow_ui(acc, acc, 2, prec);
	
	//set the highest x, y, t for the accuracy test to establish sumlim
	...
	{
		//obtain Ht(z)
		Ht_Riemann_sum_new(s, sumlimtarget, onedivsumstep, numthreads, prec);
	...
	printf(", ");
	arb_printd(te, 20);
	printf("\n\nfrom %ld..+%ld in %ld steps\n", -sumlimtarget, sumlimtarget, onedivsumstep*sumlimtarget*2);
...
int main(int argc, char *argv[])
{
    ...
    slong i, kl, kh, kr, ks, sumlim, onedivsumstep, prec, res, sw; res = 0; sumlim = 0;
	...
	onedivsumstep = 10; sumlim = 12;
	//sumlim = Establish_settings_Riemann_sum(sumlim, acc, kh, te, onedivsumstep, numthreads, prec);
	...
	Initial_fill_with_zeros_at_t0(res, kl, kh, ks, sumlim, onedivsumstep, sw, prec);
	...
	while(arb_lt(ta, te))
	{
		...
		for (i = kl; i < kh + 1; i++)
		{
			...
			FindRoots(root, xs, ys, sumlim, onedivsumstep, acc, prec);
			acb_get_real(x, root);
			acb_get_imag(y, root);
